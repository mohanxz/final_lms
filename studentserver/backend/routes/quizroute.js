...
router.post('/submit/:noteId', verifyAccessToken, async (req, res) => {
  try {
    ...
    // Determine the index for the mark based on the note type
    // 0:coding, 1:quiz, 2:assignment, 3:seminar, 4:theory, 5:practical
    let markIndex = 1; // default to quiz
    if (note.type === 'theory') markIndex = 4;
    else if (note.type === 'practical') markIndex = 5;
    else if (note.type === 'seminar') markIndex = 3;
    else if (note.type === 'assignment') markIndex = 2;

    if (!report) {
      const initialMarks = [-3, -3, -3, -3, -3, -3];
      initialMarks[markIndex] = score;
      report = new Report({
        student: student._id,
        module: note.module,
        day: note.day,
      ...
      const updatedMarks = [...(report.marksObtained || [-3, -3, -3, -3, -3, -3])];
      updatedMarks[markIndex] = score;
